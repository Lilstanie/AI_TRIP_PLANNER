...
- Numbers in tables align to the trailing edge, text to the leading edge. Tabular figures follow the existing `tabular-nums` rule in `apps/web/app/styles/base.css`.
...
Where arrangement encodes progression, as in star ratings, step indicators and progress bars, the sequence mirrors in RTL and stars fill from the trailing side. Flexbox and grid with logical properties mirror automatically; hand-positioned elements do not. Digit order inside numbers never reverses.
...
- One primary action per view. The design contract reserves `--accent` for it.
